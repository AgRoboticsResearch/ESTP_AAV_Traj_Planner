...
from . import raycast  # import raycast.RayCaster2D
from . import polygon_utils  # import polygon_utils.invert_polygon
...
def create_voxel_map_from_grid_map(grid_map: "GridMap2D"):
    occupancy_map_2d = grid_map.get_grid().T
    origin = grid_map.get_origin()
    voxel_scale = grid_map.get_resolution()
...
def draw_voxel_map_in_2d(ax: plt.Axes, voxel_map: VoxelMap, draw_virtual: bool = False):
    ...
    for idx, state in enumerate(voxels_2d):
        if state > 0:
            if not draw_virtual and state == 3:
                continue
            x = idx % grid_size[0]
    ...
    ax.imshow(map_2d, cmap=cmap, alpha=0.6, origin="lower", extent=extent)
    ax.set_xlabel("x")
    ax.set_ylabel("y")
        ...
        if use_raster:
            ...
            for polygon in self.obs_polygons:
                ...
                mask = rasterio.features.rasterize(
                    [(mapping(polygon_utils.invert_polygon(polygon)), 1)],
                    out_shape=grid_size,
                    transform=transform,
                    fill=0,
                    all_touched=True,
