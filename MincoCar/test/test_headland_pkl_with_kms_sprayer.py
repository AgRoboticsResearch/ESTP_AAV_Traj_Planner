...
from pymincocar import VehicleGeometry, KinoAstar, TrajPlanner
...
def plot_aux_circle_result(ax, circle_result):
    for center in circle_result.aux_centers:
        circle = patches.Circle(
            center, circle_result.radius, edgecolor="b", facecolor="none"
        )
        ax.add_patch(circle)
...
if __name__ == "__main__":
    ...
    pkl_path = os.path.join(os.path.dirname(__file__), "headland.pkl")
    ...
    safety_distance = 0.2
    row_width = 2.5
    # decide max sagitta based upon the feature of the aav
    # the car_width will be replaced if the implement is wider along the lateral direction
    ...
    #     [[-1.0, 4.3 / 2], 0.5, 4.3],
    # ]
    aux_poly_features = [[[-2.1,1.0/2],1.0,1.0],
                     [[-1.0,4.3/2],0.5,0.5],
                     [[-1.0,-4.3/2+0.5],0.5,0.5],]
    ...
    vehicle_model = VehicleModel(
        ...
        # car_vertices=car_vertices,
        aux_vertices_list=aux_poly_features,
        enable_aux=True,
    )

    if COLLISION_CHECK_TYPE == 0 and USE_ONE_POLY:
        convex_vertices = vehicle_model.get_conservative_convex_poly_vertices_in_body()
    else:
        convex_vertices = vehicle_model.get_car_convex_poly_vertices_in_body()
    vehicle_geo = VehicleGeometry(convex_vertices, car_wheelbase)

    aux_convex_vertices_list = vehicle_model.get_aux_convex_poly_vertices_list_in_body()
    vehicle_geo.setAuxVertices(aux_convex_vertices_list)
    ...
    # kino_a_search = KinoAstar(config_yaml, voxel_map, vehicle_geo)
    # result = kino_a_search.getCoverCircle()
    # result = vehicle_geo.getCoverCircle()
    # print(result.aux_centers)
    # plot_circle_result(ax, result)
    # plot_aux_circle_result(ax,result)
    # ax.legend(fontsize=9)
    # fig.tight_layout()
    # vehicle_model.draw_car(plt, 0, 0, 0)
    # plt.show()
    # Assign start and end state
    start_state = np.array([1.15, 3.75, np.pi, 0])
    end_state = np.array([1.2, 8.75, 0, 0])
    ...
    # collision_free = True
    # for pose in kino_search_poses:
    # for pose in kino_searchTime_poses:
    #     x, y, yaw = pose[:3]
    #     # Create a (3, 1) numpy array for the pose
    #     pos_and_yaw = np.array([[x], [y], [yaw]], dtype=np.float64)
    #     vehicle_model.draw_car(plt, x, y, yaw)

    #     # Perform collision check
    #     collision = kino_a_search.circleCheckCollisionUsingPosAndYaw(pos_and_yaw)
    #     # collision = kino_a_search.checkCollisionUsingPosAndYaw(pos_and_yaw)

    #     T = pose[:2].reshape(2, 1)
    #     R = np.array([np.cos(yaw), -np.sin(yaw), np.sin(yaw), np.cos(yaw)]).reshape(
    #         2, 2
    #     )
    #     for point in circle_result.centers:
    #         rotated_point = R @ point.reshape(2, 1)
    #         transformed_point = rotated_point + T
    #         ax.plot(transformed_point[0], transformed_point[1], "co", markersize=1)

    #     for point in circle_result.aux_centers:
    #         rotated_point = R @ point.reshape(2, 1)
    #         transformed_point = rotated_point + T
    #         ax.plot(transformed_point[0], transformed_point[1], "co", markersize=1)
    ...
    #         ax.plot(x, y, "rx")  # Mark collision points with red 'x'
    #         print(f"Collision detected at: x={x}, y={y}, yaw={yaw}")
    #     else:
    #         # ax.plot(x, y, "go")  # Mark collision-free points with green 'o'
    #         pass

    # if collision_free:
    #     print("All poses are collision-free")
    # else:
    #     print("Some poses have collisions")
    ...
    if back_end_success:
        ...
        plot_corridors(ax, traj_planner.getCorridor(), color="C1", fill=False)
        plot_corridors(ax, traj_planner.getAuxCorridor(), color="C0", fill=False)
