...
USE_ONE_POLY = False # merge the main body and aux into one poly
USE_FIRI = False

if __name__ == "__main__":
    
    """ Generate a voxel map """
    pkl_path = os.path.join(os.path.dirname(__file__), "complex_headland_case.pkl")
    ...
    feature_map = FeatureMap2D(x_min=115.60, x_max=145.60, y_min=97.506, y_max=117.50)
    ...
    # Generate grid map
    resolution = 0.1 # m
    grid_map = feature_map.to_gridmap(resolution)
    
    # Generate voxel map
    voxel_map = create_voxel_map_from_grid_map(grid_map)
    if USE_FIRI: voxel_map.dilate(1, is_virtual=False) 
    ...
    # start_state = np.array([0.5, 1.25, -np.pi, 0])
    # end_state = np.array([-4.20449058, 6.25, 0, 0])
    
    start_state = np.array([128.51, 110.82, 0, 0])
    end_state = np.array([135.00, 106.45, np.pi, 0])
    ...
    ax.plot(
        kino_search_poses[:, 0], kino_search_poses[:, 1],
        "b-o", markersize=3, alpha=0.5, label="Kino search",
    )
    ...
    ax.plot(
        kino_searchTime_poses[:, 0], kino_searchTime_poses[:, 1],
        "r-s", markersize=1, alpha=0.5, label="Kino searchTime",
    )
    
    fig2, ax2 = plt.subplots(8, 1, sharex=True, figsize=(6, 8))
    ax2[0].set_ylabel("x")
    ax2[1].set_ylabel("y")
    ...
    for i in range(3):
        if i == 2:
            ax2[i].plot(
                kino_searchTime_time, normalize_angle(kino_searchTime_poses[:, i]), "ro", markersize=1
            )
        else:
            ax2[i].plot(
    ...
    if back_end_success:
        ...
        sample_ts, sample_poses, sample_v_a_cur_omg, sample_dirs = extract_from_opt_trajs(
            opt_trajs_list, 0.1
        )

        # Plot optimal trajectories
        for pose in sample_poses:
            vehicle_model.draw_car(ax, pose[0], pose[1], pose[2])
