...
if __name__ == "__main__":
    ...
    end_state = np.array([-3.05, 8.75, 0, 0])
    ...
    # sfc = traj_planner.getCorridor()
    # aux_sfc = traj_planner.getAuxCorridor()

    # sfc = [hPoly for hPolys in sfc for hPoly in hPolys]
    # aux_sfc = [hPoly for hPolys in aux_sfc for hPoly in hPolys[0]]

    # # Extract optimal trajectories
    # constraint_points = traj_planner.getConstraintPts()

    # fig4, ax4 = plt.subplots(1, 1)
    # for i, pose in enumerate(constraint_points):
    #     ax4.clear()
    #     feature_map.draw(ax4)
    #     draw_voxel_map_in_2d(ax4, voxel_map)

    #     vehicle_model.draw_car(
    #         ax4, pose[0], pose[1], pose[2], car_color="C1", aux_color="C0"
    #     )

    #     plot_corridors(ax4, sfc[i], color='C1', fill=True, alpha=0.1)
    #     plot_corridors(ax4, aux_sfc[i], color='C0', fill=True, alpha=0.1)

    #     plt.savefig(f"./corridor_results.tmp/corridor_{i}.png")
