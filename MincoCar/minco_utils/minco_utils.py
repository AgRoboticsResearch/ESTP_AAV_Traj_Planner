...
def extract_from_opt_trajs(opt_trajs_list: List, dt=0.1):
    basetime = 0.0
    sample_ts = []
    sample_poses = []
    sample_v_a_cur_omg = []
    ...
    for idx, opt_traj in enumerate(opt_trajs_list):
        ...
        sample_t = np.arange(0, duration + dt, dt)
        # pose and v_a_cur
        for t in sample_t:
            t = min(t, duration)
    ...
    sample_v_a_cur_omg = np.array(sample_v_a_cur_omg)
    sample_ts = np.array(sample_ts).flatten()
    sample_dirs = np.array(sample_dirs).flatten()

    return sample_ts, sample_poses, sample_v_a_cur_omg, sample_dirs


def plot_corridors(ax: plt.Axes, hpolys_list: List, color="C0", fill=True, alpha=0.1):
    if isinstance(hpolys_list, np.ndarray):
        plot_corridor(ax, hpolys_list, color, fill, alpha)
        return

    for hpolys in hpolys_list:
        if isinstance(hpolys, np.ndarray):
            if not hpolys.any():
                print("corridor is empty!")
                continue
            plot_corridor(ax, hpolys, color, fill, alpha)
            continue
        else:
            for hpoly in hpolys:
                if not hpoly.any():
                    print("corridor is empty!")
                    continue
                plot_corridor(ax, hpoly, color, fill, alpha)
